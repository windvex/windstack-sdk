...
export function parsePermissionLevel(
  value: string,
): VexaniumPermissionLevel & { permissionLevel: `${string}@${string}` } {
  const candidate = value.includes("@") ? value : `${value}@active`;
  try {
    const parts = candidate.split("@");
    if (parts.length !== 2) throw new Error("Permission level must contain one separator");
    const [actor, permission] = parts as [string, string];
    if (
      !isAntelopeName(actor) ||
      !isAntelopeName(permission) ||
      `${actor}@${permission}` !== candidate
    ) {
...
export function normalizeVexaniumAccount(
  value: unknown,
  fallbackChainId: VexaniumChainId,
): VexaniumAccount {
  ...
  if (typeof value === "object" && value !== null) {
    ...
    const aliases = [source.permissionLevel, source.account, source.name].filter(
    ...
    const permissionLevel =
      aliases[0] ?? (source.actor ? `${source.actor}@${source.permission ?? "active"}` : undefined);
    ...
    if (
      aliases.some((alias) => parsePermissionLevel(alias).permissionLevel !== canonicalPermission)
    ) {
...
export function normalizeVexaniumAccounts(
  value: unknown,
  fallbackChainId: VexaniumChainId,
): VexaniumAccount[] {
