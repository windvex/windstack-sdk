...
`@windstack/rpc` provides a typed client for Antelope chain RPC endpoints. It supports multiple endpoints, automatic failover for retriable read failures, request timeouts, `AbortSignal` cancellation, injected `fetch`, structured RPC errors, table queries, account queries, ABI queries, currency queries, required-key resolution, and transaction broadcast.
