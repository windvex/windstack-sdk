...
type EVMCompatClient = {
  ...
  request<TResult = unknown, TParams = unknown>(
    args: LegacyRequestArguments<TParams>,
  ): Promise<TResult>;
...
type VexaniumCompatClient = {
  connect(args: {
    chainId: string;
    dapp?: unknown;
  }): Promise<Array<{ permissionLevel: string; label?: string }>>;
...
export type LegacyWispInvokeArgs<TParams = unknown> = {
  scope: LegacyWispScope;
  request: LegacyRequestArguments<TParams>;
};
...
function evmScopeFromHexChainId(chainId: string): `eip155:${number}` {
  if (!/^0x(?:0|[1-9a-f][0-9a-f]*)$/i.test(chainId)) {
    throw providerError(-32603, `Provider returned invalid EVM chain ID: ${chainId}`);
  }
...
function cloneSession(session: LegacyWispSession | null): LegacyWispSession | null {
  return session
    ? {
        ...
        accounts: session.accounts.map((account) => ({ ...account })),
      }
    : null;
}

function secureSessionEntropy(): string {
  ...
  throw new Error("Secure platform randomness is required to create a wallet session id");
}

export function isEVMScope(scope: string): scope is `eip155:${number}` {
  return /^eip155:(?:0|[1-9]\d*)$/.test(scope);
}

export function isVexaniumScope(scope: string): scope is `antelope:${string}` {
  return /^antelope:[0-9a-f]{32}$/.test(scope);
}

export function isSolanaScope(scope: string): scope is `solana:${string}` {
  return /^solana:[a-zA-Z0-9_-]+$/.test(scope);
}

export function createSessionId(scopes: string[]): string {
  return `wisp:${Date.now().toString(36)}:${scopes.join(",")}:${secureSessionEntropy()}`;
}
...
export async function createWispSessionClient(options: LegacyWispSessionClientOptions = {}) {
  ...
  const getEVM = async (): Promise<EVMCompatClient> => {
    if (!evm) {
      const module = await import("@windstack/evm");
      evm = (await module.createEVMClient()) as EVMCompatClient;
    }
    return evm;
  };
  ...
  const getSolana = async (): Promise<SolanaCompatClient> => {
    if (!solana) {
      const module = await import("@windstack/solana");
  ...
  const getVexanium = async (): Promise<VexaniumCompatClient> => {
    if (!vexanium) {
      const module = await import("@windstack/vexanium");
      vexanium = (await module.createVexaniumClient({
        dapp: options.dapp as never,
      })) as VexaniumCompatClient;
    }
    return vexanium;
  };
  ...
  const disconnectAll = async (): Promise<void> => {
    await Promise.allSettled(
      [evm?.disconnect?.(), solana?.disconnect(), vexanium?.disconnect()].filter(
        Boolean,
      ) as Promise<unknown>[],
    );
  };
  ...
  return {
    async connect(scopes: LegacyWispScope[]): Promise<LegacyWispSession> {
      ...
      if (
        !unique.length ||
        !unique.every(
          (scope) => isEVMScope(scope) || isSolanaScope(scope) || isVexaniumScope(scope),
        )
      ) {
        throw providerError(-32602, "One or more wallet scopes are invalid");
      }
      ...
      try {
        ...
        if (solanaScope) {
          for (const account of await (await getSolana()).connect()) {
            accounts.push({
              scope: solanaScope,
        ...
        if (vexScope) {
          ...
          if (
            !active ||
            !(active === vexScope || active.endsWith(vexScope.slice("antelope:".length)))
          ) {
            throw providerError(-32603, "Vexanium provider returned the wrong chain");
          }
          for (const account of connected) {
            accounts.push({
              scope: vexScope,
              address: account.permissionLevel,
              label: account.label,
            });
          }
        }
      ...
      if (!accounts.length) {
        await disconnectAll();
        throw providerError(4100, "No wallet accounts were authorized");
      }

      const now = Date.now();
      session = {
        id: createSessionId(unique),
        dapp: options.dapp,
        origin:
    ...
    getSession(): LegacyWispSession | null {
      return cloneSession(session);
    },

    async invoke<TResult = unknown, TParams = unknown>(
      args: LegacyWispInvokeArgs<TParams>,
    ): Promise<TResult> {
      if (!session || !session.scopes.includes(args.scope)) {
        throw providerError(-32602, "No active session for requested scope");
      }
      if (isEVMScope(args.scope)) {
        return (await getEVM()).request<TResult, TParams>(args.request);
      }
      if (isSolanaScope(args.scope)) {
        return (await getSolana()).request<TResult, TParams>(args.request);
      }
      if (isVexaniumScope(args.scope)) {
        return (await getVexanium()).request<TResult, TParams>(args.request);
      }
